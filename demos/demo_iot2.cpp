...
const std::string DB_PATH = "flsm_demo_iot2";
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " ForgeLSM Demo: High-Throughput IoT Ingestion (Large Values)\n";
    std::cout << "========================================================\n";
    std::cout << "Goal: Prove WiscKey's extremely low Write Amplification (~1.0x)\n";
    std::cout << "      when using realistically large 4KB payloads.\n\n";
    ...
    for (int t = 0; t < NUM_THREADS; ++t) {
        workers.emplace_back([db, t]() {
            // Create a 4KB payload to simulate a rich IoT sensor packet (e.g. JSON with historical array / diagnostic trace)
            std::string payload = "{\"sensor_type\":\"temperature\",\"val\":22.5,\"loc\":\"warehouse_1\",\"status\":\"ok\",\"timestamp\":1719283746,\"trace\":\"";
            payload.append(4000, '*'); // Pad with 4000 characters to make the value large
            payload += "\"}";
    ...
    std::cout << " Throughput:         " << std::fixed << std::setprecision(0) << ((NUM_THREADS * WRITES_PER_THREAD) / diff.count()) << " inserts/sec\n";
    std::cout << " Write Amplification:" << std::fixed << std::setprecision(2) << final_stats.write_amplification << "x\n";
    std::cout << " -> Notice how Write Amplification stays incredibly close to 1.0x!\n";
    std::cout << " -> Because the payload is 4KB, WiscKey separates it into the vLog (WAF=1.0x).\n";
    std::cout << " -> The LSM tree only compacts tiny keys, so its high WAF is negligible overall.\n";
    std::cout << " -> In a standard LSM-Tree (RocksDB), this 4KB value would be repeatedly compacted,\n";
    std::cout << " -> multiplying the massive disk I/O by 10x-30x!\n";
    std::cout << "========================================================\n\n";
    ...
    std::ofstream dump_file("final_iot2_dump.txt");
    ...
    for (size_t i = 0; i < std::min<size_t>(1000, all_data.size()); ++i) { // Cap at 1000 to save space
        dump_file << all_data[i].first << " : [VALUE SIZE: " << all_data[i].second.size() << " bytes]\n";
    }
    dump_file << "\n... (omitted " << (all_data.size() - std::min<size_t>(1000, all_data.size())) << " more records) ...\n";
    dump_file.close();

    std::cout << " [Data Verification] Successfully dumped the first 1,000 IoT records to 'final_iot2_dump.txt'!\n\n";
